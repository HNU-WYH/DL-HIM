...
作用�?    评估各求解器（HINTS-Fixed / AA / PAAA / ELS × Jacobi / GS）的
    时间-精度权衡（time-to-solution）与加速比�?
推荐运行方式（从项目根目录执行）�?    python scripts/compute_cost_analysis.py \
        --config helmholtz* \
        --output supplement/costs_mem_study/helmholtz_cost.pdf \
        --save-table supplement/costs_mem_study/helmholtz_speedup.md

    如果想对 diffusion 做同样分析，�?helmholtz* 换成 diffusion* 即可�?
    也可以指�?baseline（默认使�?CASES 里的第一�?case 作为 baseline）：
...
生成的文件：
    1. <output> 指定的图片（�?helmholtz_cost.pdf�?       - 上下两张子图：Error vs Time、Residual vs Time
    2. <output 前缀>_data.npz（如 helmholtz_cost_data.npz�?       - 可复现绘图的原始数据：iter、error、residual、time
    3. --save-table 指定�?markdown 文件（如 helmholtz_speedup.md�?       - 基于 baseline �?speedup 表格（error / residual 双表�?
终端输出�?    - 运行进度（evaluate_single_solver 自带�?tqdm�?    - Baseline 信息
    - Error-based Speedup �?    - Residual-based Speedup �?    - 各保存文件的路径

依赖�?    - 会自动调�?scripts.evaluate_single_solver.run_evaluation() 重新跑所�?      CASES 中的 case，并收集 wall-clock time。因此无需事先手动�?      evaluate_single_solver.py�?================================================================================
"""
import os
import argparse
import numpy as np
