...
class HybridSolver:
    def __init__(self, cfg: Box,
        ...
        self.n_dim = int(cfg.problem.n_dim)
        ...
        if self.n_dim == 1:
            ...
            self.prob_y_nodes = None
        elif self.n_dim == 2:
            if don_x_nodes is not None:
                if not isinstance(don_x_nodes, (tuple, list)) or len(don_x_nodes) != 2:
                    raise ValueError("For 2D problems, don_x_nodes must be a tuple/list: (x_nodes, y_nodes).")
                self.don_x_nodes = np.asarray(don_x_nodes[0], dtype=np.float64)
                self.don_y_nodes = np.asarray(don_x_nodes[1], dtype=np.float64)
            else:
                self.don_x_nodes, self.don_y_nodes = generate_xy_nodes(
                    cfg.data.mesh.grid_type,
                    cfg.data.mesh.grid_nx,
                    cfg.data.mesh.grid_ny,
                )

            if prob_x_nodes is not None:
                if not isinstance(prob_x_nodes, (tuple, list)) or len(prob_x_nodes) != 2:
                    raise ValueError("For 2D problems, prob_x_nodes must be a tuple/list: (x_nodes, y_nodes).")
                self.prob_x_nodes = np.asarray(prob_x_nodes[0], dtype=np.float64)
                self.prob_y_nodes = np.asarray(prob_x_nodes[1], dtype=np.float64)
            else:
                self.prob_x_nodes = np.linspace(0, 1, self.k_x.shape[0])
                self.prob_y_nodes = np.linspace(0, 1, self.k_x.shape[1])
        else:
            raise NotImplementedError(f"HybridSolver does not support n_dim={self.n_dim}.")
    ...
    def _neural_step(self, u_inner: np.ndarray, residual: Optional[np.ndarray] = None) -> np.ndarray:
        ...
        if self.n_dim == 2:
            # 2D: k_x is (Nx, Ny); residual is flat ((Nx-2)*(Ny-2),)
            Mx, My = self.k_x.shape[0] - 2, self.k_x.shape[1] - 2
            delta_u = self.model.predict(
                k_x=self.k_x, f_x=residual.reshape(Mx, My)
            ).ravel()
        else:
            # 1D: original path unchanged
    ...
    def solve(self, u_init: Optional[np.ndarray] = None,
        ...
        if u_init is None:
            if self.n_dim == 1:
                ...
                n_inner = (len(self.prob_x_nodes) - 2) * (len(self.prob_y_nodes) - 2)
            u_curr = np.zeros(n_inner, dtype=np.float64)
            ...
            if self.n_dim == 1:
                ...
                Nx, Ny = len(self.prob_x_nodes), len(self.prob_y_nodes)
                n_inner = (Nx - 2) * (Ny - 2)
                if u_init.shape == (Nx, Ny):
                    u_curr = u_init[1:-1, 1:-1].reshape(-1)
                elif u_init.size == n_inner:
                    u_curr = u_init.reshape(-1)
                elif u_init.shape == (Nx - 2, Ny - 2):
                    u_curr = u_init.reshape(-1)
                else:
                    raise ValueError("The shape of initial solution is not compatible")
