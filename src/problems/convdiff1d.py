...
class ConvectionDiffusion1D:
    """
    Solve: -eps·u''(x) + k(x)·u'(x) = f(x) with u(0)=u_left, u(1)=u_right.
    """

    def __init__(self, x_nodes, k_x, f_x, eps=1.0, mat_type="sparse", **kwargs):
        """
        Args:
            x_nodes: uniform / non-uniform grid in [0,1]  shape: (n,)
            k_x: convection parameter function            shape: (n,) or (1,)
            f_x: rhs function                             shape: (n,)
            eps: diffusion parameter function             shape: (1,)
        ...
        A_diff = build_diffusion_matrix_1d(self.x, self.eps, self.mat_type)
        A_conv = build_convection_matrix_1d(self.x, self.k, self.mat_type)
        self.A_global = A_diff + A_conv
